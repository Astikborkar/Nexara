...
import { fetchProspects, scanNewProspectApi } from '../api/client';
import { Target, Sparkles, CheckCircle2, RefreshCw, Cpu, MessageSquare, Zap } from 'lucide-react';
...
export default function AIProspecting() {
  ...
  const handleRunScanner = async () => {
    setIsScanning(true);
    setScanLog('Connecting to Express API & 6sense Intent Harvester...');
    ...
    setTimeout(async () => {
      let newProspect = await scanNewProspectApi();
      if (!newProspect) {
        newProspect = {
          id: `p-${Date.now()}`,
          name: "David Vance",
    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {isScanning && (
        <div className="glass-panel-glow" style={{ padding: '14px 20px', borderRadius: 'var(--radius-md)', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <Cpu className="spin" size={18} color="#00e699" />
          <span style={{ fontSize: '0.88rem', color: '#00e699', fontFamily: 'var(--font-mono)' }}>
            {scanLog}
          </span>
        </div>
      )}
            ...
            {['All', 'Qualified', 'Active'].map(tab => (
              <button
                ...
                style={{
                  ...
                  fontSize: '0.78rem',
                  fontWeight: '600',
                  border: filterIntent === tab ? '1px solid #00e699' : '1px solid var(--border-color)',
                  background: filterIntent === tab ? 'rgba(0, 230, 153, 0.15)' : 'rgba(255, 255, 255, 0.03)',
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: isSelected ? '1px solid rgba(0, 230, 153, 0.4)' : '1px solid var(--border-color)',
                    background: isSelected ? 'rgba(18, 28, 24, 0.95)' : 'var(--bg-card)',
                    boxShadow: isSelected ? '0 0 15px rgba(0, 230, 153, 0.25)' : 'none',
                ...
                >
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#33ffbb', background: 'rgba(0, 230, 153, 0.1)', padding: '6px 10px', borderRadius: '4px', marginBottom: '10px' }}>
          ...
          <div className="glass-panel" style={{ padding: '28px', borderRadius: 'var(--radius-md)', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid var(--border-color)', paddingBottom: '18px' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <strong style={{ fontSize: '1.1rem', color: '#00e699' }}>{selectedProspect.revenue}</strong>
              </div>
            </div>

            {/* AI Fit Factor Matrix */}
            <div>
              <h4 style={{ fontSize: '0.92rem', fontWeight: '700', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px', color: '#00e699' }}>
                ...
                {selectedProspect.scoredFactors.map((f, idx) => (
                  <div key={idx} style={{ background: 'rgba(255, 255, 255, 0.03)', padding: '12px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', fontWeight: '600', marginBottom: '6px' }}>
                      <span>{f.factor}</span>
                      <span style={{ color: '#00e699' }}>{f.score}% Score</span>
                    </div>
                    <div style={{ height: '6px', width: '100%', background: 'rgba(255, 255, 255, 0.1)', borderRadius: '3px', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${f.score}%`, background: 'linear-gradient(90deg, #00e699, #059669)', borderRadius: '3px' }}></div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div style={{ background: 'rgba(255, 255, 255, 0.03)', padding: '16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: '700', color: '#00e699', textTransform: 'uppercase' }}>Detected Tech Stack</span>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '8px' }}>
                  {selectedProspect.techStack.map((tech, i) => (
                    <span key={i} style={{ background: 'rgba(0, 230, 153, 0.15)', border: '1px solid rgba(0, 230, 153, 0.3)', padding: '3px 8px', borderRadius: '4px', fontSize: '0.75rem', color: '#33ffbb' }}>
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <div style={{ background: 'rgba(255, 255, 255, 0.03)', padding: '16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: '700', color: '#00e699', textTransform: 'uppercase' }}>Primary Intent Signal</span>
            ...
            <div style={{ background: 'linear-gradient(135deg, rgba(0, 230, 153, 0.1) 0%, rgba(14, 165, 233, 0.1) 100%)', border: '1px solid rgba(0, 230, 153, 0.3)', padding: '18px', borderRadius: 'var(--radius-sm)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <span style={{ fontSize: '0.8rem', fontWeight: '700', color: '#00e699', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <MessageSquare size={14} /> Autonomous AI Prospecting Personalization Hook
                </span>
                <span style={{ fontSize: '0.7rem', color: '#33ffbb' }}>Ready for Sequence Dispatch</span>
              </div>
              <p style={{ fontSize: '0.88rem', color: '#e2e8f0', fontStyle: 'italic', lineHeight: '1.5' }}>
